...
double guessNew(RooRealVar *mgg, RooMultiPdf *mpdf, RooCategory *mcat, RooAbsData *data, double bestPoint, double nllBest, double boundary, double massRangeLow, double massRangeHigh, double crossing, double tolerance){
  ...
  while (TMath::Abs(distanceFromTruth/crossing)>tolerance) {
    ...
    if (verbose_) {
      ...
      cout << "[INFO] \t ----------- " << distanceFromTruth/crossing << " -------------" << "distancefromtruth/crossing" << endl;
    }
    ...
    if (nIts>20) {
      return guess;
      //lowPoint = TMath::Max(0.,lowPoint-80);
...
int main(int argc, char* argv[]){
  ...
  RooRealVar *mgg = (RooRealVar*)inWS->var("hgg_mass");
  ...
  */
  ...
  if (doBands) {
	...
	double center = plot->GetXaxis()->GetBinCenter(i);
      */
      ...
      // double lowRange = 129;
      // double highRange = 122;

      TH1F *hdataInt = new TH1F("hdataInt","h",1,122.,129.);
      data->fillHistogram(hdataInt,RooArgList(*mgg) );
      // //  pdf->fillHistogram(hdataInt,RooArgList(*mass),data->sumEntries() );
      ...
      double nllBest = getNormTermNll(mgg,data,mpdf,mcat,nomBkg,lowRange,highRange);


      //      TH1F *integralhisto = (TH1F*) data->createHistogram("hgg_masshisto",*mass,1,122,129);


      //TH1F *hpdf = new TH1F("hpdf","h",1,122.,129.);
      //nomBkgCurve->fillHistogram(hpdf,RooArgList(*mgg) );


      std::cout << "[FOR TABLE] ,"<<flashggCats_[cat]<<","<< mass << ","<< nomBkg <<", assuming resolution of " << higgsResolution_ << std::endl;
      std::cout << "[FOR TABLE per GeV] ,"<<flashggCats_[cat]<<","<< mass << ","<< nomBkg_perGeV <<", assuming resolution of " << higgsResolution_ << std::endl;
      //      std::cout << "[FOR old TAB] ,"<<flashggCats_[cat]<<","<< mass << ","<<nomBkg_perGeV<<", assuming resolution of " << higgsResolution_ << std::endl; 
      ...
      std::cout << "[FOR TABLE Unblind] ,"<<flashggCats_[cat]<<","<<  mass << ","<<  hdataInt->Integral() << std::endl; 

      cout<< "[INFO] " << "mgg: " << center << " nomBkg: " << nomBkg << " lR: " << lowRange << " hR: " << highRange << endl;
      ...
      if (nomBkg<1.e-5) {
	if (verbose_) cout<< "[INFO] " << " can't handle having 0 events " << endl;
	errLow1Value = 0.;
	errLow2Value = 0.;
	...
	if (verbose_) cout<< "[INFO] " << " errLow = " << errLow1Value << " cp nomBk = " << nomBkg  << endl;
	...
	if (verbose_) cout<< "[INFO] " << "errHigh1" << endl;
	errHigh1Value = guessNew(mgg,mpdf,mcat,data,nomBkg,nllBest,highRange,lowedge,upedge,1.,nllTolerance);
	if (verbose_) cout<< "[INFO] " << " errHigh = " << errHigh1Value << " cp nomBk = " << nomBkg  << endl;
	if (verbose_) cout<< "[INFO] " << "errHigh2" << endl;
	errHigh2Value = guessNew(mgg,mpdf,mcat,data,nomBkg,nllBest,highRange,lowedge,upedge,4.,nllTolerance);
      }
      ...
      cout<< "[TABLE BkgUncert] ," << flashggCats_[cat] << "," << nomBkg << "," << errHigh1  << "," << errLow1 << endl;
  ...
  if (doSignal){
    ...
    if (SignalType==1) {
      std::cout << "SignalType 1 " << std::endl;
      TH1F::SetDefaultSumw2();
      TH1F *higgs2016Hist = (TH1F*)sigFile->Get(Form("th1f_sig_higgs_2016_mass_m125_%s",catname.c_str()));
      TH1F *higgs2017Hist = (TH1F*)sigFile->Get(Form("th1f_sig_higgs_2017_mass_m125_%s",catname.c_str()));

      TH1F *whHist = (TH1F*)sigFile->Get(Form("th1f_sig_SMS_TChiHH_HToGG_m125_%s",catname.c_str()));
      TH1F *zhHist = (TH1F*)sigFile->Get(Form("th1f_sig_zh_mass_m125_%s",catname.c_str()));

      TH1F *sigHist = (TH1F*)sigFile->Get(Form("th1f_sig_SMS_TChiHH_HToGG_m125_%s",catname.c_str())); //gghHist->Clone(Form("th1f_sig_mass_m125_%s",catname.c_str()));
      ...
      if (!MH) MH = (RooRealVar*)w_sig->var("hgg_mass");
