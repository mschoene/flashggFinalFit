...
int main(int argc, char* argv[]){
  ...
  if (saveMultiPdf){
    ...
    if (isFlashgg_){
      ...
      if (!intL){ intL = new RooRealVar("IntLumi","IntLumi",25); }
      ...
      if (!sqrts){ sqrts = new RooRealVar("SqrtS","SqrtS",13); }
      if (!intL){ intL = new RooRealVar("IntLumi","IntLumi",25); }
      std::cout << "[INFO]2 got intL and sqrts " << intL << ", " << sqrts << std::endl;
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    if (isData_) {
      dataFull = (RooDataSet*)inWS->data(Form("data_125_13TeV_%s",catname.c_str()));
	...
	dataFull->add(*dataFull0->get(),1.0);
      ...
      if (verbose) std::cout << "[INFO] opened data for  "  << Form("data_125_13TeV_%s",catname.c_str()) <<" - " << dataFull <<std::endl;
    }
    else 
