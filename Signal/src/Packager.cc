  ...
  for (int mh=mhLow_; mh<=mhHigh_; mh+=5){
    ...
    for (int cat=0; cat<nCats_; cat++) {
  	...
  	if( merge ) { 
  	  ...
  	  std::cout << "IS saved yet " << saveWS->data(Form("sig_%s_mass_m%d_%s",proc->c_str(),mh,catname.c_str())) << std::endl;
  	...
  	std::cout << "Got tempData  " << tempData << std::endl;
