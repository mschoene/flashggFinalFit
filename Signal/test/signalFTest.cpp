...
double getMyNLL(RooRealVar *var, RooAbsPdf *pdf, RooDataHist *data){
  ...
  for (int i=0; i<data->numEntries(); i++){
    double binCenter = data->get(i)->getRealValue("hgg_mass");
...
int main(int argc, char *argv[]){
  ...
  if(verbose_) std::cout << "[INFO] Got mass variable " << mass << "  " << mass->getVal() << std::endl;
  //mass->setBins(80); 
  mass->setBins(nBins);
  ...
  for (unsigned int p=0; p<procs.size(); p++){
    ...
    for (int cat=0; cat<ncats_; cat++){
      ...
      plot->SetTitle( Form("%s_%s",procs[p].c_str(),flashggCats_[cat].c_str()));
      temp.push_back(plot);
    }
  ...
  for (int cat=0; cat<ncats_; cat++){
	...
	RooDataSet *data0   = (RooDataSet*)inWS->data(
	  ...
	  mass->setVal(data0->get(i)->getRealValue("hgg_mass"));
	...
	if (verbose_) std::cout << "[INFO] Retrieved reducedWV data"<< *dataWV  << std::endl;
      ...
      std::cout << "MH= " << MH->getVal()<< "  mass= "  << mass->getVal()<< " mass_= " << mass_ <<  std::endl;
